...
import {
  ...
  familyMoodResetIdeas,
  familyThanksRelays,
  familyOutingCharmMemos,
  familyWeeklyCheerThemes,
  familyTinyAchievementBadges,
  familyBedtimePreparationMemos,
  ...
  familyHelpingHands,
  familyDiscoveryShares,
  familyWeekendCalmPlans,
  familyWeeklyCareThemes,
  familyTriedMemos,
  familyHomecomingBreathers,
  familyJournalRelayEntries,
  familyConversationTopics,
  familyAppreciationCards,
  familyRoleBatons,
  familyPlaceCards,
  familyNoticeBoards,
  familyPriorityMemos,
...
export async function createFamilyCareMessage(input: { familyGroupId: number; senderUserId: number; recipientUserId?: number; message: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const values = { ...input, recipientUserId: input.recipientUserId ?? null }; const result = await db.insert(familyCareMessages).values(values); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...values, isRead: false, readAt: null, recipientResponse: "unread" as const, responseAt: null }; }
export async function getFamilyCareMessages(familyGroupId: number, viewerUserId: number) { const db = await getDb(); return db ? db.select().from(familyCareMessages).where(and(eq(familyCareMessages.familyGroupId, familyGroupId), or(isNull(familyCareMessages.recipientUserId), eq(familyCareMessages.senderUserId, viewerUserId), eq(familyCareMessages.recipientUserId, viewerUserId)))).orderBy(desc(familyCareMessages.createdAt)) : []; }
export async function markFamilyCareMessageRead(input: { familyGroupId: number; messageId: number; readerUserId: number }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyCareMessages).set({ isRead: true, readAt: new Date(), recipientResponse: "read", responseAt: new Date() }).where(and(eq(familyCareMessages.id, input.messageId), eq(familyCareMessages.familyGroupId, input.familyGroupId), ne(familyCareMessages.senderUserId, input.readerUserId), or(isNull(familyCareMessages.recipientUserId), eq(familyCareMessages.recipientUserId, input.readerUserId)))); }
export async function deferFamilyCareMessage(input: { familyGroupId: number; messageId: number; recipientUserId: number }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyCareMessages).set({ recipientResponse: "later", responseAt: new Date() }).where(and(eq(familyCareMessages.id, input.messageId), eq(familyCareMessages.familyGroupId, input.familyGroupId), eq(familyCareMessages.recipientUserId, input.recipientUserId), ne(familyCareMessages.senderUserId, input.recipientUserId))); }
...
export async function createFamilyMoodResetIdea(input: { familyGroupId: number; userId: number; kind: "breath" | "music" | "move" | "rest"; title: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const result = await db.insert(familyMoodResetIdeas).values(input); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...input, isTried: false }; }
export async function getFamilyMoodResetIdeas(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyMoodResetIdeas).where(eq(familyMoodResetIdeas.familyGroupId, familyGroupId)).orderBy(familyMoodResetIdeas.isTried, desc(familyMoodResetIdeas.createdAt)).limit(30) : []; }
export async function updateFamilyMoodResetIdea(input: { familyGroupId: number; ideaId: number; isTried: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyMoodResetIdeas).set({ isTried: input.isTried }).where(and(eq(familyMoodResetIdeas.id, input.ideaId), eq(familyMoodResetIdeas.familyGroupId, input.familyGroupId))); }

export async function createFamilyThanksRelay(input: { familyGroupId: number; userId: number; recipientHint?: string; message: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const values = { ...input, recipientHint: input.recipientHint ?? null }; const result = await db.insert(familyThanksRelays).values(values); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...values, isReceived: false }; }
export async function getFamilyThanksRelays(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyThanksRelays).where(eq(familyThanksRelays.familyGroupId, familyGroupId)).orderBy(familyThanksRelays.isReceived, desc(familyThanksRelays.createdAt)).limit(30) : []; }
export async function updateFamilyThanksRelay(input: { familyGroupId: number; relayId: number; isReceived: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyThanksRelays).set({ isReceived: input.isReceived }).where(and(eq(familyThanksRelays.id, input.relayId), eq(familyThanksRelays.familyGroupId, input.familyGroupId))); }

export async function createFamilyOutingCharmMemo(input: { familyGroupId: number; userId: number; kind: "item" | "caution" | "cheer"; memo: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const result = await db.insert(familyOutingCharmMemos).values(input); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...input, isChecked: false }; }
export async function getFamilyOutingCharmMemos(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyOutingCharmMemos).where(eq(familyOutingCharmMemos.familyGroupId, familyGroupId)).orderBy(familyOutingCharmMemos.isChecked, desc(familyOutingCharmMemos.createdAt)).limit(30) : []; }
export async function updateFamilyOutingCharmMemo(input: { familyGroupId: number; memoId: number; isChecked: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyOutingCharmMemos).set({ isChecked: input.isChecked }).where(and(eq(familyOutingCharmMemos.id, input.memoId), eq(familyOutingCharmMemos.familyGroupId, input.familyGroupId))); }

export async function createFamilyWeeklyCheerTheme(input: { familyGroupId: number; userId: number; weekKey: string; theme: string; support?: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const values = { ...input, support: input.support ?? null }; const result = await db.insert(familyWeeklyCheerThemes).values(values); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...values, isActive: true }; }
export async function getFamilyWeeklyCheerThemes(familyGroupId: number, weekKey: string) { const db = await getDb(); return db ? db.select().from(familyWeeklyCheerThemes).where(and(eq(familyWeeklyCheerThemes.familyGroupId, familyGroupId), eq(familyWeeklyCheerThemes.weekKey, weekKey))).orderBy(familyWeeklyCheerThemes.isActive, desc(familyWeeklyCheerThemes.createdAt)).limit(20) : []; }
export async function updateFamilyWeeklyCheerTheme(input: { familyGroupId: number; themeId: number; isActive: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyWeeklyCheerThemes).set({ isActive: input.isActive }).where(and(eq(familyWeeklyCheerThemes.id, input.themeId), eq(familyWeeklyCheerThemes.familyGroupId, input.familyGroupId))); }

export async function createFamilyTinyAchievementBadge(input: { familyGroupId: number; userId: number; kind: "kindness" | "effort" | "bravery" | "care"; title: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const result = await db.insert(familyTinyAchievementBadges).values(input); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...input, isCelebrated: false }; }
export async function getFamilyTinyAchievementBadges(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyTinyAchievementBadges).where(eq(familyTinyAchievementBadges.familyGroupId, familyGroupId)).orderBy(familyTinyAchievementBadges.isCelebrated, desc(familyTinyAchievementBadges.createdAt)).limit(30) : []; }
export async function updateFamilyTinyAchievementBadge(input: { familyGroupId: number; badgeId: number; isCelebrated: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyTinyAchievementBadges).set({ isCelebrated: input.isCelebrated }).where(and(eq(familyTinyAchievementBadges.id, input.badgeId), eq(familyTinyAchievementBadges.familyGroupId, input.familyGroupId))); }

export async function createFamilyBedtimePreparationMemo(input: { familyGroupId: number; userId: number; kind: "bag" | "clothes" | "plan" | "care"; memo: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const result = await db.insert(familyBedtimePreparationMemos).values(input); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...input, isPrepared: false }; }
export async function getFamilyBedtimePreparationMemos(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyBedtimePreparationMemos).where(eq(familyBedtimePreparationMemos.familyGroupId, familyGroupId)).orderBy(familyBedtimePreparationMemos.isPrepared, desc(familyBedtimePreparationMemos.createdAt)).limit(30) : []; }
export async function updateFamilyBedtimePreparationMemo(input: { familyGroupId: number; memoId: number; isPrepared: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyBedtimePreparationMemos).set({ isPrepared: input.isPrepared }).where(and(eq(familyBedtimePreparationMemos.id, input.memoId), eq(familyBedtimePreparationMemos.familyGroupId, input.familyGroupId))); }

export async function createFamilyWellbeingNote(input: { familyGroupId: number; userId: number; state: "good" | "slow" | "tired" | "need_space"; supportNeed?: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const values = { ...input, supportNeed: input.supportNeed ?? null }; const result = await db.insert(familyWellbeingNotes).values(values); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...values }; }
export async function getFamilyWellbeingNotes(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyWellbeingNotes).where(eq(familyWellbeingNotes.familyGroupId, familyGroupId)).orderBy(desc(familyWellbeingNotes.createdAt)).limit(30) : []; }

export async function createFamilyMonthlyJoyBox(input: { familyGroupId: number; userId: number; monthKey: string; joy: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const result = await db.insert(familyMonthlyJoyBoxes).values(input); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...input, isRealized: false }; }
export async function getFamilyMonthlyJoyBoxes(familyGroupId: number, monthKey: string) { const db = await getDb(); return db ? db.select().from(familyMonthlyJoyBoxes).where(and(eq(familyMonthlyJoyBoxes.familyGroupId, familyGroupId), eq(familyMonthlyJoyBoxes.monthKey, monthKey))).orderBy(familyMonthlyJoyBoxes.isRealized, desc(familyMonthlyJoyBoxes.createdAt)).limit(30) : []; }
export async function updateFamilyMonthlyJoyBox(input: { familyGroupId: number; joyId: number; isRealized: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyMonthlyJoyBoxes).set({ isRealized: input.isRealized }).where(and(eq(familyMonthlyJoyBoxes.id, input.joyId), eq(familyMonthlyJoyBoxes.familyGroupId, input.familyGroupId))); }
...
export async function createFamilyHelpingHand(input: { familyGroupId: number; userId: number; situation: string; smallAction: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const result = await db.insert(familyHelpingHands).values(input); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...input, isHelpful: false }; }
export async function getFamilyHelpingHands(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyHelpingHands).where(eq(familyHelpingHands.familyGroupId, familyGroupId)).orderBy(familyHelpingHands.isHelpful, desc(familyHelpingHands.createdAt)).limit(30) : []; }
export async function updateFamilyHelpingHand(input: { familyGroupId: number; handId: number; isHelpful: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyHelpingHands).set({ isHelpful: input.isHelpful }).where(and(eq(familyHelpingHands.id, input.handId), eq(familyHelpingHands.familyGroupId, input.familyGroupId))); }

export async function createFamilyDiscoveryShare(input: { familyGroupId: number; userId: number; discovery: string; sourceHint?: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const values = { ...input, sourceHint: input.sourceHint ?? null }; const result = await db.insert(familyDiscoveryShares).values(values); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...values, isSaved: false }; }
export async function getFamilyDiscoveryShares(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyDiscoveryShares).where(eq(familyDiscoveryShares.familyGroupId, familyGroupId)).orderBy(familyDiscoveryShares.isSaved, desc(familyDiscoveryShares.createdAt)).limit(30) : []; }
export async function updateFamilyDiscoveryShare(input: { familyGroupId: number; shareId: number; isSaved: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyDiscoveryShares).set({ isSaved: input.isSaved }).where(and(eq(familyDiscoveryShares.id, input.shareId), eq(familyDiscoveryShares.familyGroupId, input.familyGroupId))); }

export async function createFamilyWeekendCalmPlan(input: { familyGroupId: number; userId: number; plan: string; timingHint?: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const values = { ...input, timingHint: input.timingHint ?? null }; const result = await db.insert(familyWeekendCalmPlans).values(values); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...values, isEnjoyed: false }; }
export async function getFamilyWeekendCalmPlans(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyWeekendCalmPlans).where(eq(familyWeekendCalmPlans.familyGroupId, familyGroupId)).orderBy(familyWeekendCalmPlans.isEnjoyed, desc(familyWeekendCalmPlans.createdAt)).limit(30) : []; }
export async function updateFamilyWeekendCalmPlan(input: { familyGroupId: number; planId: number; isEnjoyed: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyWeekendCalmPlans).set({ isEnjoyed: input.isEnjoyed }).where(and(eq(familyWeekendCalmPlans.id, input.planId), eq(familyWeekendCalmPlans.familyGroupId, input.familyGroupId))); }

export async function createFamilyWeeklyCareTheme(input: { familyGroupId: number; userId: number; weekKey: string; theme: string; careHint?: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const values = { ...input, careHint: input.careHint ?? null }; const result = await db.insert(familyWeeklyCareThemes).values(values); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...values, isActive: true }; }
export async function getFamilyWeeklyCareThemes(familyGroupId: number, weekKey: string) { const db = await getDb(); return db ? db.select().from(familyWeeklyCareThemes).where(and(eq(familyWeeklyCareThemes.familyGroupId, familyGroupId), eq(familyWeeklyCareThemes.weekKey, weekKey))).orderBy(familyWeeklyCareThemes.isActive, desc(familyWeeklyCareThemes.createdAt)).limit(20) : []; }
export async function updateFamilyWeeklyCareTheme(input: { familyGroupId: number; themeId: number; isActive: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyWeeklyCareThemes).set({ isActive: input.isActive }).where(and(eq(familyWeeklyCareThemes.id, input.themeId), eq(familyWeeklyCareThemes.familyGroupId, input.familyGroupId))); }

export async function createFamilyTriedMemo(input: { familyGroupId: number; userId: number; triedThing: string; reflection?: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const values = { ...input, reflection: input.reflection ?? null }; const result = await db.insert(familyTriedMemos).values(values); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...values, isKept: false }; }
export async function getFamilyTriedMemos(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyTriedMemos).where(eq(familyTriedMemos.familyGroupId, familyGroupId)).orderBy(familyTriedMemos.isKept, desc(familyTriedMemos.createdAt)).limit(30) : []; }
export async function updateFamilyTriedMemo(input: { familyGroupId: number; memoId: number; isKept: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyTriedMemos).set({ isKept: input.isKept }).where(and(eq(familyTriedMemos.id, input.memoId), eq(familyTriedMemos.familyGroupId, input.familyGroupId))); }

export async function createFamilyHomecomingBreather(input: { familyGroupId: number; userId: number; breather: string; note?: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const values = { ...input, note: input.note ?? null }; const result = await db.insert(familyHomecomingBreathers).values(values); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...values, isTaken: false }; }
export async function getFamilyHomecomingBreathers(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyHomecomingBreathers).where(eq(familyHomecomingBreathers.familyGroupId, familyGroupId)).orderBy(familyHomecomingBreathers.isTaken, desc(familyHomecomingBreathers.createdAt)).limit(30) : []; }
export async function updateFamilyHomecomingBreather(input: { familyGroupId: number; breatherId: number; isTaken: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyHomecomingBreathers).set({ isTaken: input.isTaken }).where(and(eq(familyHomecomingBreathers.id, input.breatherId), eq(familyHomecomingBreathers.familyGroupId, input.familyGroupId))); }

export async function createFamilyJournalRelayEntry(input: { familyGroupId: number; userId: number; dailyKey: string; entry: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const result = await db.insert(familyJournalRelayEntries).values(input); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...input, isPassed: false }; }
export async function getFamilyJournalRelayEntries(familyGroupId: number, dailyKey: string) { const db = await getDb(); return db ? db.select().from(familyJournalRelayEntries).where(and(eq(familyJournalRelayEntries.familyGroupId, familyGroupId), eq(familyJournalRelayEntries.dailyKey, dailyKey))).orderBy(familyJournalRelayEntries.isPassed, desc(familyJournalRelayEntries.createdAt)).limit(30) : []; }
export async function updateFamilyJournalRelayEntry(input: { familyGroupId: number; entryId: number; isPassed: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyJournalRelayEntries).set({ isPassed: input.isPassed }).where(and(eq(familyJournalRelayEntries.id, input.entryId), eq(familyJournalRelayEntries.familyGroupId, input.familyGroupId))); }

export async function createFamilyConversationTopic(input: { familyGroupId: number; userId: number; topic: string; note?: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const values = { ...input, note: input.note ?? null }; const result = await db.insert(familyConversationTopics).values(values); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...values, isDiscussed: false }; }
export async function getFamilyConversationTopics(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyConversationTopics).where(eq(familyConversationTopics.familyGroupId, familyGroupId)).orderBy(familyConversationTopics.isDiscussed, desc(familyConversationTopics.createdAt)).limit(30) : []; }
export async function updateFamilyConversationTopic(input: { familyGroupId: number; topicId: number; isDiscussed: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyConversationTopics).set({ isDiscussed: input.isDiscussed }).where(and(eq(familyConversationTopics.id, input.topicId), eq(familyConversationTopics.familyGroupId, input.familyGroupId))); }

export async function createFamilyAppreciationCard(input: { familyGroupId: number; userId: number; recipientName: string; message: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const result = await db.insert(familyAppreciationCards).values(input); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...input, isSeen: false }; }
export async function getFamilyAppreciationCards(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyAppreciationCards).where(eq(familyAppreciationCards.familyGroupId, familyGroupId)).orderBy(familyAppreciationCards.isSeen, desc(familyAppreciationCards.createdAt)).limit(30) : []; }
export async function updateFamilyAppreciationCard(input: { familyGroupId: number; cardId: number; isSeen: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyAppreciationCards).set({ isSeen: input.isSeen }).where(and(eq(familyAppreciationCards.id, input.cardId), eq(familyAppreciationCards.familyGroupId, input.familyGroupId))); }

export async function createFamilyRoleBaton(input: { familyGroupId: number; userId: number; task: string; nextPerson?: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const values = { ...input, nextPerson: input.nextPerson ?? null }; const result = await db.insert(familyRoleBatons).values(values); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...values, isCompleted: false }; }
export async function getFamilyRoleBatons(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyRoleBatons).where(eq(familyRoleBatons.familyGroupId, familyGroupId)).orderBy(familyRoleBatons.isCompleted, desc(familyRoleBatons.createdAt)).limit(30) : []; }
export async function updateFamilyRoleBaton(input: { familyGroupId: number; batonId: number; isCompleted: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyRoleBatons).set({ isCompleted: input.isCompleted }).where(and(eq(familyRoleBatons.id, input.batonId), eq(familyRoleBatons.familyGroupId, input.familyGroupId))); }

export async function createFamilyPlaceCard(input: { familyGroupId: number; userId: number; placeName: string; reason?: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const values = { ...input, reason: input.reason ?? null }; const result = await db.insert(familyPlaceCards).values(values); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...values, isVisited: false }; }
export async function getFamilyPlaceCards(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyPlaceCards).where(eq(familyPlaceCards.familyGroupId, familyGroupId)).orderBy(familyPlaceCards.isVisited, desc(familyPlaceCards.createdAt)).limit(30) : []; }
export async function updateFamilyPlaceCard(input: { familyGroupId: number; placeId: number; isVisited: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyPlaceCards).set({ isVisited: input.isVisited }).where(and(eq(familyPlaceCards.id, input.placeId), eq(familyPlaceCards.familyGroupId, input.familyGroupId))); }

export async function createFamilyNoticeBoard(input: { familyGroupId: number; userId: number; notice: string; detail?: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const values = { ...input, detail: input.detail ?? null }; const result = await db.insert(familyNoticeBoards).values(values); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...values, isAcknowledged: false }; }
export async function getFamilyNoticeBoards(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyNoticeBoards).where(eq(familyNoticeBoards.familyGroupId, familyGroupId)).orderBy(familyNoticeBoards.isAcknowledged, desc(familyNoticeBoards.createdAt)).limit(30) : []; }
export async function updateFamilyNoticeBoard(input: { familyGroupId: number; noticeId: number; isAcknowledged: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyNoticeBoards).set({ isAcknowledged: input.isAcknowledged }).where(and(eq(familyNoticeBoards.id, input.noticeId), eq(familyNoticeBoards.familyGroupId, input.familyGroupId))); }

export async function createFamilyPriorityMemo(input: { familyGroupId: number; userId: number; priority: string; note?: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const values = { ...input, note: input.note ?? null }; const result = await db.insert(familyPriorityMemos).values(values); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...values, isResolved: false }; }
export async function getFamilyPriorityMemos(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyPriorityMemos).where(eq(familyPriorityMemos.familyGroupId, familyGroupId)).orderBy(familyPriorityMemos.isResolved, desc(familyPriorityMemos.createdAt)).limit(30) : []; }
export async function updateFamilyPriorityMemo(input: { familyGroupId: number; memoId: number; isResolved: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyPriorityMemos).set({ isResolved: input.isResolved }).where(and(eq(familyPriorityMemos.id, input.memoId), eq(familyPriorityMemos.familyGroupId, input.familyGroupId))); }

export async function createFamilyPlanCheckin(input: { familyGroupId: number; userId: number; plan: string; supportNote?: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const values = { ...input, supportNote: input.supportNote ?? null }; const result = await db.insert(familyPlanCheckins).values(values); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...values, isConfirmed: false }; }
export async function getFamilyPlanCheckins(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyPlanCheckins).where(eq(familyPlanCheckins.familyGroupId, familyGroupId)).orderBy(familyPlanCheckins.isConfirmed, desc(familyPlanCheckins.createdAt)).limit(30) : []; }
export async function updateFamilyPlanCheckin(input: { familyGroupId: number; checkinId: number; isConfirmed: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyPlanCheckins).set({ isConfirmed: input.isConfirmed }).where(and(eq(familyPlanCheckins.id, input.checkinId), eq(familyPlanCheckins.familyGroupId, input.familyGroupId))); }

export async function createFamilyNextStepCard(input: { familyGroupId: number; userId: number; step: string; reason?: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const values = { ...input, reason: input.reason ?? null }; const result = await db.insert(familyNextStepCards).values(values); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...values, isTaken: false }; }
export async function getFamilyNextStepCards(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyNextStepCards).where(eq(familyNextStepCards.familyGroupId, familyGroupId)).orderBy(familyNextStepCards.isTaken, desc(familyNextStepCards.createdAt)).limit(30) : []; }
export async function updateFamilyNextStepCard(input: { familyGroupId: number; cardId: number; isTaken: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyNextStepCards).set({ isTaken: input.isTaken }).where(and(eq(familyNextStepCards.id, input.cardId), eq(familyNextStepCards.familyGroupId, input.familyGroupId))); }

export async function createFamilyHouseholdTip(input: { familyGroupId: number; userId: number; tip: string; category?: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const values = { ...input, category: input.category ?? null }; const result = await db.insert(familyHouseholdTips).values(values); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...values, isHelpful: false }; }
export async function getFamilyHouseholdTips(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyHouseholdTips).where(eq(familyHouseholdTips.familyGroupId, familyGroupId)).orderBy(familyHouseholdTips.isHelpful, desc(familyHouseholdTips.createdAt)).limit(30) : []; }
export async function updateFamilyHouseholdTip(input: { familyGroupId: number; tipId: number; isHelpful: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyHouseholdTips).set({ isHelpful: input.isHelpful }).where(and(eq(familyHouseholdTips.id, input.tipId), eq(familyHouseholdTips.familyGroupId, input.familyGroupId))); }

export async function createFamilyPackingCheck(input: { familyGroupId: number; userId: number; item: string; occasion?: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const values = { ...input, occasion: input.occasion ?? null }; const result = await db.insert(familyPackingChecks).values(values); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...values, isChecked: false }; }
export async function getFamilyPackingChecks(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyPackingChecks).where(eq(familyPackingChecks.familyGroupId, familyGroupId)).orderBy(familyPackingChecks.isChecked, desc(familyPackingChecks.createdAt)).limit(30) : []; }
export async function updateFamilyPackingCheck(input: { familyGroupId: number; checkId: number; isChecked: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyPackingChecks).set({ isChecked: input.isChecked }).where(and(eq(familyPackingChecks.id, input.checkId), eq(familyPackingChecks.familyGroupId, input.familyGroupId))); }

export async function createFamilyTogetherPick(input: { familyGroupId: number; userId: number; pick: string; kind?: string }) { const db = await getDb(); if (!db) throw new Error("Database not available"); const values = { ...input, kind: input.kind ?? null }; const result = await db.insert(familyTogetherPicks).values(values); return { id: Number((result as { insertId?: number }).insertId ?? 0), ...values, isEnjoyed: false }; }
export async function getFamilyTogetherPicks(familyGroupId: number) { const db = await getDb(); return db ? db.select().from(familyTogetherPicks).where(eq(familyTogetherPicks.familyGroupId, familyGroupId)).orderBy(familyTogetherPicks.isEnjoyed, desc(familyTogetherPicks.createdAt)).limit(30) : []; }
export async function updateFamilyTogetherPick(input: { familyGroupId: number; pickId: number; isEnjoyed: boolean }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyTogetherPicks).set({ isEnjoyed: input.isEnjoyed }).where(and(eq(familyTogetherPicks.id, input.pickId), eq(familyTogetherPicks.familyGroupId, input.familyGroupId))); }
