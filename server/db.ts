...
export async function markFamilyCareMessageRead(input: { familyGroupId: number; messageId: number; readerUserId: number }) { const db = await getDb(); if (!db) throw new Error("Database not available"); return db.update(familyCareMessages).set({ isRead: true, readAt: new Date() }).where(and(eq(familyCareMessages.id, input.messageId), eq(familyCareMessages.familyGroupId, input.familyGroupId), ne(familyCareMessages.senderUserId, input.readerUserId), or(isNull(familyCareMessages.recipientUserId), eq(familyCareMessages.recipientUserId, input.readerUserId)))); }
