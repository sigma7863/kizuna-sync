...
export function FamilyCardNavigator({ onOpen, role = "guardian" }: { onOpen: (cardId: string) => void; role?: FamilyDiscoveryRole }) {
  ...
  const saveNavigatorState = (group: string, expanded: boolean, sort: DiscoverySortMode) => {
    try { window.localStorage.setItem(NAV_STATE_KEY, JSON.stringify({ group, expanded, sortMode: sort })); } catch { /* storage is optional */ }
  };
  ...
  return <Card id="family-card-navigator" className="mb-6 scroll-mt-4 border-0 bg-gradient-to-br from-slate-900 via-indigo-950 to-violet-950 text-white shadow-lg">
    ...
    <CardContent className="space-y-4">
      ...
      {showExplorer && <div className="space-y-3 border-t border-white/15 pt-4">
        ...
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{results.map((card) => <div key={card.id} title={getCardHint(card)} className="group flex items-center gap-2 rounded-xl bg-white/10 p-3 transition hover:bg-white/20"><button type="button" onClick={() => open(card.id)} className="min-w-0 flex-1 text-left"><span className="flex items-center gap-2 text-sm font-semibold"><span>{card.title}</span>{card.featured && <Badge className="bg-violet-300/25 text-[10px] text-violet-100 hover:bg-violet-300/25">注目</Badge>}{newCardIds.includes(card.id) && <Badge className="bg-emerald-300/25 text-[10px] text-emerald-100 hover:bg-emerald-300/25">新着</Badge>}</span><span className="mt-1 block text-xs text-indigo-100">{card.description}</span></button><button type="button" onClick={() => { const next = toggleFavoriteCard(favoriteIds, card.id); setFavoriteIds(next); try { window.localStorage.setItem(FAVORITES_KEY, JSON.stringify(next)); } catch { /* storage is optional */ } }} className={`rounded-lg p-1.5 transition ${favoriteIds.includes(card.id) ? "text-rose-300" : "text-indigo-200 hover:text-rose-200"}`}><Heart className={`h-4 w-4 ${favoriteIds.includes(card.id) ? "fill-current" : ""}`}/></button><ArrowRight className="h-4 w-4 shrink-0 text-violet-200"/></div>)}</div>
      </div>}
    </CardContent>
  </Card>;
}
